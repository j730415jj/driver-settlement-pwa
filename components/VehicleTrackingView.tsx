...
// Leaflet CDN 스타일 및 스크립트 로드를 위한 유틸리티
const loadLeaflet = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveVehicles(prev => prev.map(v => {
        ...
        const newSpeed = Math.max(0, (v.speed || 0) + (Math.random() - 0.5) * 10);
        return {
          ...v,
          lat: (v.lat || 36.019) + dLat,
          lng: (v.lng || 129.343) + dLng,
    ...
    activeVehicles.forEach(vehicle => {
      ...
      try {
        ...
        if (marker) {
          ...
          marker.bindPopup(`<strong>${vehicle.vehicleNo}</strong><br>차주: ${vehicle.ownerName}<br>속도: ${Math.round(vehicle.speed || 0)}km/h`);
    ...
    <div className="flex h-[calc(100vh-140px)] gap-6 overflow-hidden">
      ...
      <div className="w-80 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col no-print">
        <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
          ...
          <span className="text-[10px] text-gray-400 font-medium tracking-tighter">30분 주기</span>
        </div>
