...
const DashboardView: React.FC<Props> = ({ operations, vehicles, dispatches }) => {
  // 현재 날짜 기준 월 식별자 (전역적으로 사용 가능하도록 useMemo 밖으로 이동)
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 p-4">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm font-medium">실시간 운영 지표 및 재무 현황 (입력된 모든 지출 합산)</p>
        </div>
          ...
          <div className="mt-8 p-5 bg-blue-600 dark:bg-blue-900/40 rounded-[1.5rem] text-white relative overflow-hidden">
            <div className="relative z-10">
              ...
              <p className="text-[11px] font-medium leading-relaxed opacity-90">
                기사님이 직접 기록한 <span className="font-black">매일의 지출</span>이 실시간으로 이익 계산에 반영되고 있습니다.
              </p>
            </div>
          </div>
        </div>
